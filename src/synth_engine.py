...
Includes live NOAA/Open-Meteo telemetry coupling, 24-hour diurnal circadian transitions,
tropical rain generators, black sand/volcanic pebble acoustics, blowholes, hollow barrels,
and nocturnal coastal crickets.
"""
...
from .live_data import fetch_live_telemetry, compute_dsp_multipliers, determine_circadian_phase
...
class RainGenerator:
    ...
    Synthesizes stochastic Poisson raindrop impacts, high-frequency water surface sizzle,
    and distant rolling Pacific thunder rumbles.
    """
    def __init__(self, sample_rate=SAMPLE_RATE):
        ...
        self.sos_rain_spray = signal.butter(2, 2400.0, btype='highpass', fs=self.sample_rate, output='sos')
        self.zi_spray = np.zeros((self.sos_rain_spray.shape[0], 2, 2))
        
        # Bandpass filter for droplet water surface impacts (3.2 kHz - 8.5 kHz)
        self.sos_droplets = signal.butter(2, [3200.0, 8500.0], btype='bandpass', fs=self.sample_rate, output='sos')
        self.zi_droplets = np.zeros((self.sos_droplets.shape[0], 2, 2))
        
        # Lowpass filter for distant squall thunder / atmospheric pressure roll (55 Hz)
        self.sos_thunder = signal.butter(2, 55.0, btype='lowpass', fs=self.sample_rate, output='sos')
    ...
    def generate(self, n_samples, pink_chunk, white_chunk, brown_chunk):
        ...
        spray, self.zi_spray = signal.sosfilt(self.sos_rain_spray, pink_chunk, axis=0, zi=self.zi_spray)
        ...
        droplets, self.zi_droplets = signal.sosfilt(self.sos_droplets, droplets_raw, axis=0, zi=self.zi_droplets)
        ...
        if self.current_gain > 0.4:
            ...
            thunder_audio, self.zi_thunder = signal.sosfilt(self.sos_thunder, thunder_raw, axis=0, zi=self.zi_thunder)
...
class NightFaunaGenerator:
    ...
    Procedural Hawaiian coastal tree crickets & cicadas (Laupala) pulsating in coastal kiawe groves.
    """
    def __init__(self, sample_rate=SAMPLE_RATE):
        self.sample_rate = sample_rate
    ...
    def generate(self, n_samples):
        ...
        # Fast cricket pulse rhythm (~14 Hz amplitude modulation)
        ...
        # Slower breathing cluster modulation (~0.25 Hz)
...
class WaveEvent:
    """A single procedural ocean wave with swell, crest, dump, foam sizzle, backwash drag, and coastal acoustics."""
    ...
    def step(self, n_samples, dt, pink_chunk, white_chunk, brown_chunk):
        ...
        # 3. Hollow Reef Barrel Resonance (Honolua Point Break)
        ...
        if hollow_gain > 0.01:
            ...
            if np.any(hollow_mask):
                ...
                hollow_env[hollow_mask] = (np.sin(np.pi * np.clip(tau_h / 2.1, 0, 1)) ** 2.0) * hollow_gain * self.intensity

        # 4. Shorebreak Thud (Low-end impact impulse when wave crashes into sand/lava ledges)
        thud_env = np.zeros(n_samples)
        thud_mask = (t_rel_break >= 0) & (t_rel_break < 1.2)
        if np.any(thud_mask):
            tau_thud = t_rel_break[thud_mask]
            thud_env[thud_mask] = np.sin(2 * np.pi * 55.0 * tau_thud) * np.exp(-tau_thud * 5.0) * self.preset.get('shorebreak_thud', 0.3) * self.intensity

        # 5. Volcanic Blowhole Cavern Surge (Waʻānapanapa)
        ...
        if self.has_blowhole:
            ...
            if np.any(bh_mask):
                tau_bh = t_bh[bh_mask]
                # Deep whooshing sub-bass impulse + air jet hiss
                blowhole_env[bh_mask] = np.sin(2 * np.pi * 42.0 * tau_bh) * np.exp(-tau_bh * 3.2) * self.preset.get('blowhole_gain', 0.5) * self.intensity

        # 6. Swash Foam Fizz (Billions of bursting microbubbles spreading up beach)
        ...
        # 7. Backwash Undertow Drag (Water percolating through coral & sand grains back to sea)
        ...
        # 8. Basalt Pebble & Shingle Drag Texture (Waʻānapanapa Black Sand)
        pebble_gain = self.preset.get('pebble_drag_gain', 0.0)
        pebble_env = np.zeros(n_samples)
        if pebble_gain > 0.01 and np.any(bw_mask):
            # Granular rattling of black volcanic pebbles as water recedes
        ...
        wave_l = (layer_swell[:, 0] + layer_breaker[:, 0] + layer_hollow[:, 0] + layer_foam[:, 0] + 
                  layer_bw[:, 0] + layer_pebble[:, 0] + layer_thud[:, 0] + layer_blowhole[:, 0]) * pan_l
        wave_r = (layer_swell[:, 1] + layer_breaker[:, 1] + layer_hollow[:, 1] + layer_foam[:, 1] + 
                  layer_bw[:, 1] + layer_pebble[:, 1] + layer_thud[:, 1] + layer_blowhole[:, 1]) * pan_r
...
class HawaiianOceanSynthesizer:
    ...
    Supports real-time NOAA marine buoy telemetry, circadian diurnal shifts,
    tropical rain squalls, and smooth preset morphing / shuffle.
    """
    ...
    PRESETS = {
        ...
        'honolua': {
            ...
            'shorebreak_thud': 0.35,
            'outer_reef_gain': 0.45,
        ...
        'waianapanapa': {
            ...
            'wave_period_min': 10.0,
            'wave_period_max': 15.5,
            'breaker_gain': 1.10,
            'breaker_decay': 0.42,
            'foam_gain': 0.85,
            'foam_decay': 0.25,
            ...
            'wildlife_prob': 0.15,
            'wildlife_type': 'seabird',
            'shorebreak_thud': 0.75,
            'outer_reef_gain': 0.20,
            'pebble_drag_gain': 0.75,
            'blowhole_gain': 0.65,
    ...
    def __init__(self, preset_name='napili', sample_rate=48000, enable_live=True, circadian_mode='auto', rain_intensity=None):
        ...
        # Initial live telemetry sync
        if self.enable_live or self.circadian_mode == 'auto':
            self.refresh_telemetry(force_sync=False)

    def set_preset(self, preset_name):
        """Morphs active acoustic characteristics to a new beach preset."""
        if preset_name in self.PRESETS:
            self.preset_name = preset_name
            self.preset = dict(self.PRESETS[preset_name])

    def refresh_telemetry(self, force_sync=False):
        """Fetches live buoy/weather data and updates DSP multipliers."""
        try:
            t = fetch_live_telemetry(force_refresh=force_sync)
            self.last_telemetry = t
            if self.enable_live:
                self.live_multipliers = compute_dsp_multipliers(t)
            
            # Update rain
            if self.forced_rain_intensity is not None:
                self.rain_gen.set_intensity(self.forced_rain_intensity)
            elif self.enable_live:
                self.rain_gen.set_intensity(self.live_multipliers.get('rain_intensity', 0.0))
    ...
    def _spawn_bird_call(self):
        """Generates species-accurate procedural calls (Kōlea, Koaʻe kea, or Noio/Seabirds)."""
        bird_type = self.preset.get('wildlife_type', 'kolea')
        
        if bird_type == 'koae_kea':
            # Koaʻe kea (White-tailed Tropicbird) of Kauaʻi sea cliffs / Honolua:
            ...
            # Noio (Brown Noddy) / Shearwater of Molokini & North Shore:
            # Soft churring oceanic trill (1.8 kHz - 2.6 kHz)
    ...
    def generate_chunk(self, n_samples=4096):
        ...
        swell_mult = self.live_multipliers.get('swell_gain_mult', 1.0) * getattr(self, 'diurnal_swell_mult', 1.0)
        wind_mult = self.live_multipliers.get('wind_gain_mult', 1.0) * getattr(self, 'diurnal_wind_mult', 1.0)
        bird_prob_mult = getattr(self, 'diurnal_bird_mult', 1.0)
        ...
        if self.time_to_next_wave <= 0:
            if self.in_set:
                ...
                self.active_waves.append(WaveEvent(self.preset, intensity=intensity, period_mult=period_mult))
                ...
                if self.waves_spawned_in_set >= self.waves_in_current_set:
                    ...
                    self.time_to_next_wave = random.uniform(self.preset['wave_period_min'], self.preset['wave_period_max']) * period_mult
            else:
            ...
            # Daytime bird spawn
            if (random.random() < self.preset.get('wildlife_prob', 0.15) * bird_prob_mult) and bird_prob_mult > 0.05:
        ...
        self.wind_phase1 += 2 * np.pi * lfo_spd * dt_chunk
        self.wind_phase2 += 2 * np.pi * (lfo_spd * 1.618) * dt_chunk
        ...
        # 6. Environmental Layers (Tropical Rain & Nocturnal Crickets)
